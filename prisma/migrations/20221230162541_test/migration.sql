...
-- AlterTable
ALTER TABLE `Node` ADD COLUMN `connectedFromNodeId` INTEGER NULL,
    ADD COLUMN `nodeId` INTEGER NULL;

-- DropTable
DROP TABLE `NodeConnections`;

-- AddForeignKey
ALTER TABLE `Node` ADD CONSTRAINT `Node_connectedFromNodeId_fkey` FOREIGN KEY (`connectedFromNodeId`) REFERENCES `Node`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;
